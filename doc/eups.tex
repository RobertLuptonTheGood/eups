...
Options:
   ...
   -r dir     Location of manifests and tarballs (may be URL)
   -v         Be chatty
...
Once you've created a package with \code{eups\_distrib -c} you can move the
package directory to another machine and use \code{eups\_distrib -x} to
recreate a working \eups{} installation.

\begin{verbatim}
eups_distrib -v -z act -c pslib RHL_v1_1
./configure --with-eups=/u/rhl/ppp; make install
eups_distrib -Z /u/rhl/ppp -v -z rhl -x -r /u/act/products/packages pslib RHL_v1_1
\end{verbatim}

When extracting (\code{-x}) the package root may be specified as a URL, e.g.
\hfil\break\hspace*{2cm}
\code{eups\_distrib -r http://www.astro.princeton.edu/$\sim$act/packages}
