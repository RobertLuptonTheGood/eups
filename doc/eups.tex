...
Version names are taken to be of the form \code{PrefixAAA.BBB.CCC-XXX.YYY.ZZZ}
where \code{Prefix} is optional, as is \code{-XXX.YYY.ZZZ}.  Let's call
\code{PrefixAAA.BBB.CCC} the `primary' part, and \code{XXX.YYY.ZZZ}
the `secondary' part.
...
\begin{enumerate}
  ...
  \item If one of the two versions is longer, and matches the shorter to its end,
...
(Here compar is the value that C's \code{qsort} uses;  -1 if $v_1$ is less
