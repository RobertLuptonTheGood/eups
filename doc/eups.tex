...
\item[root]
   The directory associated with a package (if any).  For the common case of
   a package containing source code the root would usually contain include and library subdirectories,
   although this is not required.
\item[tag]
   A name attached to a set of one or more packages, which can be used to override the version that would otherwise be selected
\item[version]
   A string specifying which version of a package is being used.  Conventionally these correspond to names
   generated from your source code manager (e.g. \code{svn}), but this is not required.
\item[VRO]
  The \code{V}ersion \code{R}esolution \code{O}rder;  the order in which possible versions of a package are
  considered.
\end{description}
...
are existed are also listed.  You'll see that there are two sorts of file;  \file{startup.py} and \file{config.properties}.  You should ignore the latter, and they are not further documented here.
...
For example, I have
\begin{verbatim}
import eups, os, pwd, sys

user = pwd.getpwuid(os.geteuid())[0]

hooks.config.Eups.globalTags += ["test"]
hooks.config.Eups.userTags = [user]
hooks.config.Eups.reservedTags += ["stable"]
hooks.config.Eups.VRO = {
    "commandLineVersion" : "path version warn:2 stable current",
    "default" :             { "default" : "path versionExpr current",
                              "dss" :     "path version current",
                              #"lsst" :    "path stable+versionExpr warn versionExpr current",
                              "lsst" :    "path current warn versionExpr",
                             },
    "rhl" :                "path rhl current version warn:0 versionExpr",
    }
\end{verbatim}
...
and then typed:
\begin{verbatim}
eups distrib create -v -s /u/act/products/packages --build ~/ACT/BuildFiles: --current moby &&
