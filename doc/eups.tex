...
Options:
   ...
   -c         Create a package
   -F         Force recreation/unpacking of tarballs
   -f flavor  Select a flavor (default: `eups_flavor`)
   -n         Don't actually do anything
   -r dir     Location of manifests and tarballs (may be a URL)
   -v         Be chatty
   -x         Extract and install a product from package
...
Once you've created a package with \code{eups\_distrib -c} you can log on to
another machine and use \code{eups\_distrib -x} to
recreate a working \eups{} installation.  For example:
\begin{verbatim}
eups_distrib -v -z act -c moby v1_2
./configure --with-eups=/u/rhl/ppp; make install
eups_distrib -v -x -r http://www.astro.princeton.edu/$\sim$act/packages moby v1_2
\end{verbatim}

When creating an distribution, tarballs that already present are not recreated
(unless you specify \code{-F}),
so repeating a \code{eups\_distrib -c} command to create a manifest for another
product won't lead to duplication of tarballs.

When unpacking a distribution with \code{eups\_distrib -x}, products that are
already declared to \eups{} won't be unpacked and re-declared (unless you specify \code{-F}).
...
relying on files installed in e.g. \code{/usr/local}.
