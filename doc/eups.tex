...
Options:
   -B, --build     arg   Use specified build file (@ => use product's installed build file)
...
with the table file, \code{eups distrib --build buildFilePath --create} will set things up
so that a subsequent \code{eups distrib --install} will copy the build files
to the new machine and execute it;  if you have a set of dependent products
they will be executed in the proper order.

The \code{buildFilePath} is a standard \code{:}-separated unix path; the entry
\code{@} means, ``The product's installed ups directory''.  A common choice
would be \code{--build .:@}.
