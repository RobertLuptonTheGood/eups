...
\begin{itemize}
  \item{\code{buildCache|clearCache|listCache}}
    Manipulate \eups's caches.  You can disable the use of caches entirely with an entry \code{hooks.config.Eups.disablePickleCache = True} in a \file{startup.py} file
...
required. \textit{N.b.  this will be quite slow if you've disabled the cache --- it'll need to read every table
  file that \eups knows about.}.  This cache is unrelated to the one that \text{e.g.} \code{eups admin clearCache} uses.
...
\begin{itemize}
  ...
  \item Cache files
    ...
    more important to you than speed considerations.  In the absence of such an option,
    you can put \code{hooks.config.Eups.disablePickleCache = True} into a \file{startup.py} file
