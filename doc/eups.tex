...
\code{--recursive} then all dependent products will also be removed --- note
that this is dangerous as they may be needed by someone else.  You are strongly
advised to use \code{eups uses} to check that nothing bad will happen
before you do this.
...
that depends on product is returned, along with the version required.
...
\section{``Transport Layers'' for \code{eups build}}
\label{pacman}
