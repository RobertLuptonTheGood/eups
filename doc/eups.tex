...
Options:
   -B, --build           Use product's build files
...
%------------------------------------------------------------------------------
  ...
  \item \code{eups\_expandbuild}
\begin{verbatim}
Usage:
    eups_expandbuild [options] -V version buildFile [outDir]
Options:
...
This isn't an integral part of eups, but it can be very useful in conjunction with \code{eups\_distrib}.
...
they will be executed in the proper order.  At their simplest, build files could simply
...
There's a subtlety here.  Imagine that we have a product \code{prod2}
which depends in \code{prod1}.
If we source each build file separately,
the \code{setup -r .} in \code{prod1}'s build file isn't still active by
...
tries to set itself up, which will request the \emph{current} version
of \code{prod1}.  We don't want \code{eups distrib} to always declare
products current, so what do we do?  The answer is that \code{eups distrib}
inserts explicit setup statements into the the build script as it
executes it, and replaces the \code{setup} with \code{setup -j}; try
