...
You can use a callback function to specify a set of tags that should always be used; there's an example in
section \ref{defaultTags}.
...
If you don't specify any tags (with either \code{--tag} (\code{-t}) or \code{--postTag} (\code{-T})) then
...
You can disable this default tag processing with \code{setup -t None} (or \code{setup -t ""}).
