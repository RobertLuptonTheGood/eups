...
\textbf{RHL is working on this section, and it's getting simpler}
...
\texttt{cd \$HOME/myProduct}\hfil\break
\texttt{setup gsl; setup fftw3}\hfil\break
...
\texttt{setup gsl} and \texttt{setup fftw3} in order to build
\texttt{myProduct}. The answer, is: ``It depends''.  If you're
\texttt{myProduct}'s loving author, you knew that you needed
\texttt{gsl} as you included e.g. \texttt{gsl/gsl\_rng.h} in your
source. If you're not the author, then \texttt{myProduct} presumably
includes a properly written table file (probably called
\texttt{myProduct/ups/myProduct.table}) which knows about
\texttt{gsl} and \texttt{fftw3}:
\begin{verbatim}
setupRequired("fftw3")
setupRequired("gsl")
\end{verbatim}
in this case what you \textit{really} typed was:\hfil\break
\texttt{setup --root \$HOME/myProduct myProduct}\hfil\break
which setup \texttt{gsl} and \texttt{fftw3} for you:
\begin{verbatim}
Setting up:      fftw3               Flavor: Linux      Version: v3_0_1
Setting up:      gsl                 Flavor: Linux      Version: v1_8
\end{verbatim}

The \texttt{make declare} command told \eups{} about your newly built
product; the \texttt{setup myProduct v2\_4} command told the system
...
\begin{itemize}
  ...
  \item\texttt{UPS\_WITHOUT\_CONFIGURE([fftw3], [fftw3.h], -lfftw3f, [fftw3f,fftwf\_plan\_dft\_2d])}
...
Declares \texttt{version} of \texttt{product} into the database given by \texttt{\$EUPS\_PATH/ups\_db}.
The product root directory is where the product has been installed, and can either
be an absolute pathname or specified relative to \texttt{\$EUPS\_PATH}.
...
variable \texttt{EUPS\_PATH/ups\_db} (or the \texttt{-Z} option).
...
\begin{itemize}
  ...
  \item \texttt{EUPS\_PATH}
    The root directory under which all products are installed.
    \footnote{
      Despite the name, \texttt{\$EUPS\_PATH} (currently) only allows you to specify a
      single directory rather than a :-separated list.  We intend to fix this in a future
      release}
    The UPS database is \texttt{EUPS\_PATH/ups\_db}
\end{itemize}
...
consists of a directory \texttt{\$EUPS\_PATH/ups\_db}, with a subdirectory for each declared product.
  ...
  \item{\texttt{AC\_DEFINE\_ROOT(version [, flavor])}}
    ...
    \begin{itemize}
      \item{\texttt{--with-ups=dir}}
        Specify the value of \texttt{EUPS\_PATH}
        (default: \texttt{\$EUPS\_PATH} if set, or \texttt{\$prefix/share}).
...
\end{verbatim} %$
If you have defined \texttt{\$EUPS\_PATH}, it'll be used as the default
value instead of \texttt{/usr/local/share}.
...
\end{verbatim}
\texttt{EUPS\_PATH=...} also works as an alternative to \texttt{--with-ups=...};
note upper case.

\subsection{Obselete features}

\begin{itemize}
  \item The environment variable \texttt{\$EUPS\_PATH} used to be known as \texttt{\$PROD\_DIR\_PREFIX}.

  \item The \eups{} database used to be given by the environment variable \texttt{\$PRODUCTS}
    with a default value of \texttt{\$PROD\_DIR\_PREFIX/ups\_db}.

  \item There used to be an option \texttt{-z} to set the value of \texttt{\$PRODUCTS}.
\end{itemize}
