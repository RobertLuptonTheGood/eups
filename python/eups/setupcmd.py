...
"""
    ...
    def execute(self):
        ...
        if self.opts.productDir:
            try:
                productName = eups.guessProduct(os.path.join(self.opts.productDir, "ups"), productName)
...
def guessProduct(dir, productName=None):
    ...
    if not os.path.isdir(dir):
        ...
        raise EupsException("%s isn't a directory" % dir)
            
    productNames = map(lambda t: re.sub(r".*/([^/]+)\.table$", r"\1", t), glob.glob(os.path.join(dir, "*.table")))

    if not productNames:
        raise EupsException("I can't find any table files in %s" % dir)

    if productName:
        if productName in productNames:
            return productName
        else:
            raise EupsException("You chose product %s, but I can't find its table file in %s" % (productName, dir))
