...
class VersionCallback(object):
    ...
    def set(self, callback):
        ...
        The arguments are the two version strings and the usual comparison function, and the return value is
        -1, 0, or 1.  Throw ValueError if you want to declare that the strings don't sort.
        """
        VersionCallback.callback[0] = callback
    ...
    def clear(self):
        """Clear the version callback"""
        VersionCallback.callback[0] = None
...
class Product(object):
    ...
    def initFromDirectory(self, productDir):
        ...
        if not os.path.exists(table):
            table = "/dev/null"
        self.table = Table(table).expandEupsVariables(self)
...
def guessProduct(dir, productName=None):
    ...
    if not os.path.isdir(dir):
        if productName:
            return productName          # we'll have to trust them
